...
export default function CreateGroupe({ onClose, clubs, cancel }: props) {
    const { register, handleSubmit, formState: { errors, isSubmitting }, reset } = useForm<GroupeAbonnementType>({ resolver: zodResolver(GroupeAbonnements) as any })
    const onSubmit = async (groupe: GroupeAbonnementType) => {
        try {
            const res = await groupeAbonnementsService.create(groupe)
                ...
                <div className="bg-white rounded-lg w-96 shadow-lg relative max-h-[97vh] flex flex-col">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="overflow-y-auto px-6 pb-6 pr-4">
                        ...
                        <div className="mb-3">
                            ...
                            <select
                                {...register("brancheId", { valueAsNumber: true })}
                                className="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:border-red-500 focus:ring-1 focus:ring-red-500 focus:outline-none block w-full p-2.5"
                            >
